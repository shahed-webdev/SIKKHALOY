                    ...
                    <asp:UpdatePanel ID="UpdatePanel2" runat="server">
                        <ContentTemplate>
                            ...
                            <asp:SqlDataSource ID="AttendanceSQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>"
                                SelectCommand="SELECT Attendance_Record.AttendanceDate, Attendance_Record.Reason, CreateClass.Class, StudentsClass.RollNo, Student.StudentsName, Student.SMSPhoneNo, Student.ID, CONVERT(varchar(15), 
Attendance_Record.EntryTime, 100) AS EntryTime, CONVERT(varchar(15), Attendance_Record.ExitTime, 100) AS ExitTime, Attendance_Record.Attendance, Attendance_Record.StudentClassID, 
Attendance_Record.StudentID FROM Attendance_Record INNER JOIN CreateClass ON Attendance_Record.ClassID = CreateClass.ClassID INNER JOIN StudentsClass ON Attendance_Record.StudentClassID = StudentsClass.StudentClassID INNER JOIN Student ON Attendance_Record.StudentID = Student.StudentID
WHERE (Student.Status = 'Active') AND (Attendance_Record.Attendance LIKE @Attendance) AND (Attendance_Record.SchoolID = @SchoolID) AND (Attendance_Record.EducationYearID = @EducationYearID) AND (StudentsClass.ClassID = @ClassID or @ClassID = 0) and (StudentsClass.SectionID LIKE @SectionID) and (StudentsClass.ShiftID LIKE @ShiftID) AND (StudentsClass.SubjectGroupID LIKE @SubjectGroupID) ORDER BY CASE WHEN ISNUMERIC(StudentsClass.RollNo) = 1 THEN CAST(REPLACE(REPLACE(StudentsClass.RollNo, '$', ''), ',', '') AS INT) ELSE 0 END"
