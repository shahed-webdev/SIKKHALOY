<%@ Page Title="" Language="C#" MasterPageFile="~/Basic_Authority.Master" AutoEventWireup="true" CodeBehind="Autho_Category.aspx.cs" Inherits="EDUCATION.COM.Authority.Authority_PageLink.Autho_Category" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    ...
    <asp:GridView ID="CategoryGridView" runat="server" AutoGenerateColumns="False" DataKeyNames="LinkCategoryID" DataSourceID="CategorySQL" CssClass="mGrid">
        <Columns>
            ...
            <asp:HyperLinkField DataNavigateUrlFields="LinkCategoryID" DataNavigateUrlFormatString="Autho_Sub_Category.aspx?Category={0}" DataTextField="Category" HeaderText="Select Category to Set URL" />
        </Columns>
    </asp:GridView>
    <asp:SqlDataSource ID="CategorySQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" DeleteCommand="DELETE FROM [Authority_Link_Category] WHERE [LinkCategoryID] = @LinkCategoryID"
         InsertCommand="INSERT INTO [Authority_Link_Category] ([Ascending], [Category]) VALUES (@Ascending, @Category)" SelectCommand="SELECT LinkCategoryID, Category, Ascending FROM Authority_Link_Category ORDER BY Ascending" UpdateCommand="UPDATE [Authority_Link_Category] SET [Ascending] = @Ascending, [Category] = @Category WHERE [LinkCategoryID] = @LinkCategoryID">
        <DeleteParameters>
