<%@ Page Title="Session based Paid" Language="C#" MasterPageFile="~/BASIC.Master" AutoEventWireup="true" CodeBehind="Session_Paid_Report.aspx.cs" Inherits="EDUCATION.COM.Accounts.ReportsSession.Session_Paid_Report" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <link href="CSS/Session_Student.css" rel="stylesheet" />
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    <h3 class="mb-2">Session based Paid report <small class="Date"></small></h3>
            ...
            <div id="Print-Set" class="row">
                ...
                <div class="col-sm-4">
                    <div class="Fees-section Payorder-bg">
                        <div class="tile-description">Paid</div>
                        <div class="tile-number"><%#Eval("Paid","{0:N0}") %> Tk</div>
                    </div>
                </div>
                <div class="col-sm-4">
    ...
    <asp:SqlDataSource ID="Stu_P_SQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" SelectCommand="SELECT        COUNT(DISTINCT Income_PayOrder.StudentID) AS Total_Stu, SUM(Income_PayOrder.PaidAmount) AS Paid,ROUND((SUM(Income_PayOrder.PaidAmount)* 100)/(SUM(Income_PayOrder.PaidAmount)+ SUM(Income_PayOrder.Receivable_Amount)),2) AS Percentage
...
SUM(Income_PayOrder.PaidAmount) AS Paid,ROUND((SUM(Income_PayOrder.PaidAmount)* 100)/(SUM(Income_PayOrder.PaidAmount)+ SUM(Income_PayOrder.Receivable_Amount)),2) AS Percentage
FROM  Income_PayOrder INNER JOIN StudentsClass ON Income_PayOrder.StudentClassID = StudentsClass.StudentClassID INNER JOIN Student ON Income_PayOrder.StudentID = Student.StudentID INNER JOIN
CreateClass ON StudentsClass.ClassID = CreateClass.ClassID WHERE  (Income_PayOrder.SchoolID = @SchoolID) AND (Income_PayOrder.EducationYearID = @EducationYearID) AND 
			 (Income_PayOrder.Is_Active = 1) AND 
			 (Income_PayOrder.EndDate BETWEEN ISNULL(@From_Date, '1-1-1000') AND ISNULL(@To_Date, '1-1-3000')) and 
			 (Income_PayOrder.RoleID like @RoleID) AND 
...
ORDER BY Income_PayOrder.ClassID, StudentsClass.RollNo"
            ...
            <SelectParameters>
                ...
                <asp:ControlParameter ControlID="SectionDropDownList" Name="SectionID" PropertyName="SelectedValue" />
                <asp:ControlParameter ControlID="RoleDropDownList" Name="RoleID" PropertyName="SelectedValue" />
                <asp:ControlParameter ControlID="PayForDropDownList" PropertyName="SelectedValue" Name="PayFor"></asp:ControlParameter>
            </SelectParameters>
        </asp:SqlDataSource>
    </div>
