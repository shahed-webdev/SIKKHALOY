<%@ Page Title="Session based Due" Language="C#" MasterPageFile="~/BASIC.Master" AutoEventWireup="true" CodeBehind="Session_Due_Report.aspx.cs" Inherits="EDUCATION.COM.Accounts.ReportsSession.Session_Due_Report" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <link href="CSS/Session_Student.css" rel="stylesheet" />
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    <h3 class="mb-2">Session based Due report <small class="Date"></small></h3>
    ...
    <div class="form-inline NoPrint">
        ...
        <div class="form-group">
            ...
            <asp:SqlDataSource ID="ClassNameSQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" SelectCommand="SELECT DISTINCT CreateClass.ClassID, CreateClass.Class
FROM            CreateClass INNER JOIN
                         Income_PayOrder ON CreateClass.ClassID = Income_PayOrder.ClassID
WHERE        (Income_PayOrder.SchoolID = @SchoolID) AND (Income_PayOrder.EducationYearID = @EducationYearID) AND (Income_PayOrder.Receivable_Amount &lt;&gt; 0)
ORDER BY CreateClass.ClassID">
        ...
        <div class="form-group">
            ...
            <asp:SqlDataSource ID="PayForSQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" SelectCommand="SELECT PayFor FROM Income_PayOrder WHERE (SchoolID = @SchoolID) AND (EducationYearID = @EducationYearID) AND (ClassID = @ClassID) AND (RoleID LIKE @RoleID) AND (Is_Active = 1) AND (EndDate BETWEEN ISNULL(@From_Date, '1-1-1000') AND ISNULL(@To_Date, '1-1-3000')) GROUP BY PayFor ORDER BY MAX(EndDate)" CancelSelectOnNullParameter="False">
                <SelectParameters>
                    <asp:SessionParameter Name="SchoolID" SessionField="SchoolID" />
                    <asp:ControlParameter ControlID="Session_DropDownList" Name="EducationYearID" PropertyName="SelectedValue" />
                    <asp:ControlParameter ControlID="ClassDropDownList" Name="ClassID" PropertyName="SelectedValue" />
            ...
            <div id="Print-Set" class="row">
                ...
                <div class="col-sm-4">
                    <div class="Fees-section Payorder-bg">
                        <div class="tile-description">No. of Payorder</div>
                        <div class="tile-number"><%#Eval("NumberOf_Unpaid_PayOrder") %></div>
                    </div>
                </div>
                <div class="col-sm-4">
                    <div class="Fees-section Concession-bg">
                        <div class="tile-description">Due</div>
    ...
    <asp:SqlDataSource ID="Stu_P_SQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" SelectCommand="SELECT        COUNT(DISTINCT Income_PayOrder.StudentID) AS Total_Stu, COUNT(DISTINCT Income_PayOrder.PayOrderID) AS NumberOf_Unpaid_PayOrder, SUM(Income_PayOrder.Receivable_Amount) AS Unpaid
...
WHERE        (Income_PayOrder.SchoolID = @SchoolID) AND 
			 ...
			 (Income_PayOrder.Receivable_Amount &lt;&gt; 0)"
    ...
    <div class="table-responsive">
        <asp:GridView ID="StudentDetailsGV" runat="server" AutoGenerateColumns="False" CssClass="mGrid" DataSourceID="StudentDetailsSQL" AllowSorting="True" AllowPaging="True" PageSize="100">
            <Columns>
                ...
                <asp:BoundField DataField="NumberOf_Unpaid_PayOrder" HeaderText="Due Payorder" SortExpression="NumberOf_Unpaid_PayOrder" />
                <asp:BoundField DataField="Unpaid" HeaderText="Due" SortExpression="Unpaid" />
            </Columns>
        ...
        <asp:SqlDataSource ID="StudentDetailsSQL" runat="server" ConnectionString="<%$ ConnectionStrings:EducationConnectionString %>" SelectCommand="SELECT        Income_PayOrder.StudentID, Income_PayOrder.ClassID, Student.ID, Student.StudentsName, CreateClass.Class, StudentsClass.RollNo,
                       COUNT(DISTINCT Income_PayOrder.PayOrderID) AS NumberOf_Unpaid_PayOrder, SUM(Income_PayOrder.Receivable_Amount) AS Unpaid
FROM            Income_PayOrder INNER JOIN
...
ORDER BY Income_PayOrder.ClassID, StudentsClass.RollNo"
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var PayFor = "";
            if ($('[id*=PayForDropDownList] :selected').index() > 0) {
                PayFor = " Pay For : " + $('[id*=PayForDropDownList] :selected').text() + ".";
            }
            ...
            $(".Class_Section").text(Class + Section + PaymentRole + PayFor);
