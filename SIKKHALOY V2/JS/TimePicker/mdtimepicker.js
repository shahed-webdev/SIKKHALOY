...
+function ($) {
    ...
    var MDTimePicker = function (input, config) {
        ...
        var time;
        if (that.input.val() !== '') {
            time = that.parseTime(that.input.val(), that.config.format);

            that.setValue(time);
        } else {
            time = that.getSystemTime();
    ...
    MDTimePicker.prototype = {
        ...
        setup: function (picker) {
            ...
            for (var i2 = 0; i2 < 60; i2++) {
                var min = i2 < 10 ? '0' + i2 : i2, deg2 = (MIN_START_DEG + (i2 * MIN_DEG_INCR)) % END_DEG,
					minute = $('<div class="mdtp__digit rotate-' + deg2 + '" data-minute="' + i2 + '"></div>');

                if (i2 % 5 === 0) minute.addClass('marker').html('<span>' + min + '</span>');
